...
func (d *decoder) processSOF(n int) error {
	...
	if d.tmp[5] != nComponent {
		return errors.New("SOF has wrong number of image components")
	}
	for i := 0; i < nComponent; i++ {
		...
		if i == 0 {
			if hv != 0x11 && hv != 0x21 && hv != 0x22 {
				return errors.New("luma downsample ratio")
			}
		} else {
			if hv != 0x11 {
				return errors.New("chroma downsample ratio")
			}
		}
	}
	return nil
...
func (d *decoder) processSOS(n int) error {
	...
	if d.tmp[0] != nComponent {
		return errors.New("SOS has wrong number of image components")
	}
	...
	for i := 0; i < nComponent; i++ {
		...
		if cs != d.comps[i].c {
			return errors.New("scan components out of order")
		}
		...
		for mx := 0; mx < mxx; mx++ {
				...
				for j := 0; j < d.comps[i].h*d.comps[i].v; j++ {
					...
					for k := 1; k < blockSize; k++ {
						...
						if val1 != 0 {
							k += int(val0)
							if k > blockSize {
								return errors.New("bad DCT index")
							}
			...
			if d.ri > 0 && mcu%d.ri == 0 && mcu < mxx*myy {
				...
				if d.tmp[0] != 0xff || d.tmp[1] != expectedRST {
					return errors.New("bad RST marker")
				}
				expectedRST++
